...
Alongside the audio processor, this project also includes an **ARMv8 emulator and assembler implemented entirely in C**.

The emulator models the core components of an ARMv8 CPU, including its registers, program counter, condition flags, and memory, allowing ARM instructions to be executed and tested without physical ARM hardware.

The assembler uses a **two-pass design** to resolve labels and symbols before encoding instructions into their corresponding 32-bit ARMv8 machine-code representations. This provides the foundation for assembling programs that can then be executed by the emulator.

Together, the emulator and assembler provide a low-level systems programming component to the project, complementing the hardware-focused audio processor with experience in **CPU architecture, instruction encoding, memory, and systems programming in C**.
